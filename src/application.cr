...
module Earl
  ...
  # (e.g. log agents) expect that `Earl.application` will be started. It can be
  # spawned in the background (and forgotten) or better: be leveraged to monitor
  # the program's agents and block the main `Fiber` until the program is told to
  ...
  class Application < Supervisor
    ...
    # List of POSIX signals to trap. Defaults to `SIGINT` and `SIGTERM`.
    #
